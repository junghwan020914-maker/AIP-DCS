...
## 🔴 08-14 **시드 집합 경고 — 이 진단 전체가 시드 0~29에만 해당한다**

같은 prev 매치업인데 시드 집합에 따라 결과가 두 배 넘게 갈린다:

    시드      승/패/무    격추  피격추  승점
    0~29     5/12/3        4     **7**   6.5
    30~59   15/ 4/1       11     **2**  15.5

"prev가 최저값이고 우리가 7번 죽는다"는 서술은 **인샘플 시드에만 해당**한다.
두 집합 평균은 11/20 정도로, 여전히 약한 매치업이지만 인샘플이 보여준 만큼
심각하지는 않다. **이 도구로 진단할 때는 반드시 두 시드 집합을 모두 돌릴 것.**

## ❌ 처방(방어 중 전추력)은 기각됐다 — 방향이 시드마다 반대다

    피격추   인샘플 7 -> 6 (개선)   홀드아웃 2 -> **5** (악화)
    코어 4상대  76.0 -> **72.0** (v32 -3.0, v29 -3.0)

인샘플 개선은 시드 적합이었다. 진단(스테이션키핑 폴백이 걸린다)은 유효하지만
그 처방은 강한 상대를 해친다.
