...
## ✅ 08-15 결과 — **모델은 동일하다. `info` 체력이 한 틱 늦을 뿐이다.**

1차 판정은 "모델이 다르다"(최대 절대차 1.132e-02)였다. 그러나 원인을 파보니 아니었다.

**틱 단위 대조**(env가 `env.target_damage` / `env.ownship_damage`로 노출하는 자기
틱별 피해 대 내 `score_rate`):

    arcD  seed0   불일치 **0 / 3221**    누적 1.011856 == 1.011856
    v42   seed1   불일치 **0 / 11999**   누적 0.00524803 == 0.00524803

**15,220틱 전수에서 한 틱도 안 틀린다.** 내 재계산은 시뮬레이터와 같은 식이다.

차이의 정체는 `info`의 **한 틱 지연**이다. 체력 궤적을 보면 명확하다:

    step 11988  틱피해 4.076e-04  틱합 0.00040759  체력기준 0.00000000
    step 11999  틱피해 4.065e-04  틱합 0.00524803  체력기준 0.00484153

**step N의 `1 - info["health"]`는 step N-1까지의 틱합과 정확히 같다.**
`info`는 이번 스텝의 피해가 반영되기 전 체력을 담는다. 그래서 마지막에 읽으면 항상
마지막 한 틱이 빠지고, 격추 판은 그때 틱당 피해가 커서(d_wez~0.53 -> 0.0088) 차이도 크다.
1.132e-02는 격추 순간 한 틱치였다.

➡️ **과거 수치는 모두 유효하다.** 다만 파이프라인은 정직하게 바꾼다 —
   앞으로 로컬 피해는 **`env.target_damage` / `env.ownship_damage`를 틱마다 합산**한다.
   시뮬레이터가 실제로 적용한 값 그 자체이고, 값은 재계산과 동일함이 위에서 확인됐다.
   ⚠️ `1 - info["health"]`는 쓰지 말 것 — 한 틱 늦다.
