...
## ❌ 08-15 측정 — **기각. Phase 2/3은 탈출구가 아니다.**

20시드, 1승 4패 15무(8.5/20, `_suite_eval`과 일치):

    Phase별 피해 s/판        P1        P2        P3        합
      나                 0.02025   0.00468   0.00455   0.02948
      상대                0.00000   0.00091   0.00085   0.00176

    후반 순수 기하 체류 s/판        나      상대    비
      t>=100s  ATA<2도           0.078    0.014   0.18
      t>=150s  ATA<3도           0.161    0.080   0.50

**전체로 보면 우리가 압도한다** — 피해 17배, sub-2도 체류 5.6배. 그런데:

    패배 4판의 마진   중앙 0.00742  최소 0.00525  최대 0.01520
      그 판 상대 피해   P1 0.00000  P2 0.00456  P3 0.00426
      그 판 내 피해     P1 0.00000  P2 0.00000  P3 0.00000
      후반 sub각 체류   나 **0.000s**   상대 0.471s

**지는 판에서 우리는 정확히 0이다.** "적다"가 아니라 **한 번도 3도 안에 못 들어간다.**
전체 우위는 전부 다른 16판(특히 이긴 1판)에 몰려 있다.

➡️ 사전 등록 기준 두 개 중 하나만 통과했다.
   · 마진 중앙 0.00742 < 0.01 → **통과**(HP로 0.74다. 소수점 이하 차이로 진다)
   · 후반 체류비 < 2.0 → **탈락**(우리가 0이라 비율이 무한)
   판정은 "**마진은 작으나 그들이 뒤에 눌러앉아 있다 — 기하를 바꿔야 한다**"이다.

⚠️ **이 두 번째 기준을 미리 박아두지 않았다면 정반대 결론을 냈을 것이다.**
   "HP 0.74 차이로 진다, 아주 작은 개선이면 뒤집힌다"는 이야기는 너무 그럴듯하다.
   그러나 마진이 작은 건 우리가 근접해서가 아니라 **그들의 기회도 0.47초뿐**이어서다.
   우리는 0초다. 여기서 필요한 건 정밀도가 아니라 **기회 자체**다.
   `_cross_probe.py`의 "필요 개선폭 133도 = 기하 문제"와 같은 벽이고,
   Phase 2/3이 헐겁다고 해서 그 벽이 낮아지지는 않는다.
...
def main():
    ...
    if L:
        ...
        if tight and near:
            ...
            rt = "무한(우리는 0초)" if A < 1e-9 else f"{B/A:.2f}배"
            print(f"     마진은 작으나 후반 체류가 {rt}다 — 그들이 뒤에 눌러앉아 있다.")
