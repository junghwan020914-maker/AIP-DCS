 ...
 void UCPPBehaviorTree::RunCPPBT(Vector3& VP, float& Throttle, bool& AimmingMode)
{
	...
	try
	{
		...
		// ❌ 08-14 **받음각 편향 보정 기각** — 코드는 제거하고 기록만 남긴다.
		// [진단] `_aimerr_probe.py`(arcD 8시드, 공세+밴드 11745표본): 조준 오차의 **90%가
		//   고도 축**이고(|az| 0.34도 vs |el| 1.92도), 부호반전율 0.58%로 진동이 아니며,
		//   상대각속도 상관 +0.084로 지연도 아니다. 회귀 **el ≈ -0.90*|AOA| + 1.16**.
		//   우리 받음각 중앙이 8.3도이므로 그것만으로 Phase1(ATA<1도)을 벗어난다.
		// [처방] 위치 이력으로 비행경로를 잡아 기수와의 각(=받음각)만큼 VP를 아래로 기울임.
		// [결과] **단조 악화**. arcD 6시드, 순수조준률:
		//     K=0(기존) **7.94%** 득점 1.89s | K=0.5 2.25% 1.31s
		//     K=1.0      1.38%     1.01s     | K=1.5 0.67% 0.69s
		//   부수 효과는 컸다 — 공세밴드가 23.8 -> 102.8s로 **4.3배**(아래를 겨누면
		//   오버슛하지 않고 계속 붙는다). 그러나 조준률이 더 크게 떨어져 곱이 손해다.
		// [교훈] `Functions.cpp`에 적어둔 경고에 또 걸렸다 —
		//   **관측된 상관을 정책으로 바꾸면 그 상관을 만들던 조건 자체가 사라진다.**
		//   조준점을 내리자 궤적이 달라져 el~AOA 관계가 성립하지 않게 됐다.
		//   이 프로젝트에서 상관 기반 처방은 **5전 5패**다.
