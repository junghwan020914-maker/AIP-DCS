...
/*
	...
	✅ 08-10 3차 채택 (SpeedBelow=235 / AltAbove=3000 / DistBeyond=1000 / DiveSlope=0.5 /
	   FloorAlt=2000 / **TargetAboveMax=400**, 시간제한 없음). 코어 144.5 -> 145.5.
	   1차 붕괴의 기전이 "우리가 내려가는 동안 상대가 올라가 수직으로 이별"이었으므로
	   가르는 조건은 시간이 아니라 **상대가 위에 있는가**였다.

	✅ 08-10 **동일 C++ A/B로 재확인.** `prev` 감시자에 전용 XML을 주고 나서야
	   "트리만 다르고 제어기는 같은" 비교가 처음 가능해졌다. 같은 4상대 30시드:
	       상대   e3 이전 트리   e3 트리
	       v32       27.5      **28.5**
	       v29       26.0      **27.0**
	       arcA      30.0        30.0
	       arcE      30.0        30.0
	       합계     113.5      **115.5**  (+2.0)
	   원래의 144.5 -> 145.5보다 신뢰도가 높다(제어기 변화가 섞이지 않았다).
	   ⚠️ **트리 변경을 판정할 때는 기준선도 같은 C++로 재측정할 것.** 같은 트리인데
	   옛 기준선은 28.5/26.5, 재측정은 27.5/26.0이다 — 1.5점쯤이 C++ 드리프트와 노이즈였다.

	⚠️ **알려진 약점(비이행성)**: e3 트리는 새 `prev`(e3 이전 트리 + 동일 C++)와 직접
	   붙으면 **3승 5패로 진다.** 하강하지 않는 근접 미러 상대에게 고도를 내주기 때문이다.
	   그런데 공통 상대군에서는 +2.0으로 앞선다 — 게임 AI에서 흔한 비이행성이다.
	   대회는 우리 이전 버전이 아니라 40개 미지 상대와 싸우므로 다상대 지표를 따랐다.
