...
oPlaneData ChangeData(int ID, int Team, float HP, int OperType, oNavigationData& NaviData)
{
    ...
    // BUG FIX: LocationX/Y/Z were raw Lat/Lon(deg)/Alt(m) passed straight through as if
    // already Cartesian meters. Every downstream Distance/LOS/AspectAngle/BFM calc treated
    // degrees as meters, so horizontal separation was ~1e5x too small and got swamped by
    // altitude alone. Convert through the (already-implemented, previously unused for this
    // path) LLAtoCartesian using the same shared BaseLLA reference as GetStick() below.
    BT_Geometry::Vector3 Cartesian = LLAtoCartesian(LLA, Vector3(37.91455691666666, 128.18188127777776, 0));
